...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--outputs-root", default=DEFAULT_OUTPUTS_ROOT, help="Répertoire racine de sortie")
    p.add_argument(
        "--skip-extraction",
        action="store_true",
...
def _step_compare(
    ...
    compare_main([
        "--bank", bank,
        "--year-current", str(year_current),
        "--quarter-current", quarter_current,
        "--config", config,
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("=" * 70)
    print(f"  VIGILANCE — Pipeline Batch")
    print(f"  Banque:              {bank}")
