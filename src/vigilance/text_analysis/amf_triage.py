"""Triage AMF des changements textuels et compatibilite legacy."""
...
from .constants import _TRIAGE_BATCH_SIZE, _TRIAGE_SEMANTIC_TEXT_LIMIT, _TRIAGE_SOURCE_SNIPPET_LIMIT
from .openai_gateway import _call_structured_completion_with_correction, _truncate_prompt_text
from .text_normalization import _json_dumps, _sanitize_explanation
from .vigie_objectives import (
    _attach_vigie_objective_context,
    _detect_vigie_objectives_for_change,
    _vigie_objectives_prompt,
)
...
def _triage_section_changes(
    *,
    ...
    if len(changes) > _TRIAGE_BATCH_SIZE:
        ...
        total_batches = (len(changes) + _TRIAGE_BATCH_SIZE - 1) // _TRIAGE_BATCH_SIZE
        for start in range(0, len(changes), _TRIAGE_BATCH_SIZE):
            batch_index = (start // _TRIAGE_BATCH_SIZE) + 1
            logger.info(
                "triage batch section=%s batch=%d/%d changes=%d-%d total=%d",
                section_key,
                batch_index,
                total_batches,
                start + 1,
                min(start + _TRIAGE_BATCH_SIZE, len(changes)),
                len(changes),
            )
    ...
    for idx, change in enumerate(changes, start=1):
        objective_matches = _detect_vigie_objectives_for_change(change)
    ...
    system_prompt = (
        ...
        "multi-label si plusieurs thèmes s'appliquent. Les objectifs de vigie "
        "ci-dessous servent à formuler les justifications dans l'affichage "
        "existant, sans créer une sortie séparée et sans inventer de changement."
    )
    ...
    user_prompt = (
        ...
        "Langue de sortie : rédige tous les champs textuels libres en français "
        "professionnel, notamment explanation, nouvelle_idee_justification, "
        "justification_posture et impact_it_justification. Les clés JSON, les "
        "codes AMF, les raisons d'exclusion et les valeurs d'énumération restent "
        "exactement au format technique demandé.\n\n"
        ...
        "Règles métier :\n"
        "0. OBJECTIFS DE VIGIE : lorsque objective_matches est non vide, "
        "mentionner le sujet de vigie concerné dans `Sujet détecté` ou "
        "`Point de surveillance`. Ne jamais créer un changement seulement "
        "parce qu'un objectif existe; l'objectif sert uniquement à expliquer "
        "et prioriser pourquoi le changement détecté est utile à la vigie. "
        "Si le changement est une vraie information ajoutée, retirée ou "
        "substantiellement modifiée sur un objectif de vigie, le traiter comme "
        "un point à mettre en avant: nouvelle_idee=true, impact_level au "
        "minimum MODERE, et action_requise au minimum investigation. Exception: "
        "si le changement est seulement une reformulation, un déplacement sans "
        "effet analyste, du formatage ou une variation chiffrée propre à la "
        "banque, garder is_relevant=false et expliquer pourquoi il ne faut pas "
        "le prioriser.\n"
    ...
    for idx, change in enumerate(changes, start=1):
        ...
        enriched_change = _attach_vigie_objective_context(enriched_change)
