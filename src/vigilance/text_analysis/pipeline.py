"""Orchestration du pipeline texte extraction et comparaison."""
...
from .amf_triage import _triage_section_changes
from .constants import UNIFIED_TEXT_SCHEMA_VERSION, _SECTION_LABELS, _T4_TEXT_TARGET_SECTIONS
...
from .openai_gateway import _build_openai_client
from .section_resolution import _allowed_target_sections, _extract_audits_for_pdf, _resolve_sections
from .text_change_detection import _compare_section_texts
from .text_summary import _build_global_summary, _consolidate_section_observations_with_llm, _is_retained_text_change, _retained_change_sort_key
...
def run_text_analysis_pipeline(
    ...
    * Les appels GPT du flux texte n'envoient pas de limite de complétion explicite par défaut, afin de privilégier l'arrêt naturel du modèle.
    ...
    for section_key in section_keys:
        text_t1 = _extract_section_text_from_markdown(md_previous, section_key)
        text_t2 = _extract_section_text_from_markdown(md_current, section_key)
        logger.info("text comparison section=%s start", section_key)
        ...
        logger.info("text comparison section=%s produced_changes=%d", section_key, len(changes))
        ...
        logger.info(
            "text triage section=%s non_unchanged_changes=%d",
            section_key,
            len(non_unchanged),
        )
        ...
        retained = [change for change in enriched if _is_retained_text_change(change)]
        retained.sort(key=_retained_change_sort_key)
        all_observations = _consolidate_section_observations_with_llm(
            client,
            model=model,
            section_key=section_key,
            changes=all_changes,
        ...
        retained_observations = [
            ...
            if _is_retained_text_change(observation)
        ]
        ...
        section_comparisons.append(
            {
                ...
                "observation_comparisons": retained_observations,
                "all_observation_comparisons": all_observations,
                "summary": {
    ...
    payload["global_summary"] = _build_global_summary(
        section_comparisons,
        bucket="observation_comparisons",
    )
