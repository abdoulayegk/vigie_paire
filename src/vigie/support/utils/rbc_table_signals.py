...
def build_rbc_first_column_signals(
    *,
    ...
    indicators_clean = [
        normalized
        for normalized in (normalize_indicator_for_comparison(label) for label in final_indicator_raw)
        if normalized
    ]
    groups_clean = [
        normalized
        for normalized in (normalize_indicator_for_comparison(label) for label in _uniq(group_rows))
        if normalized
    ]
