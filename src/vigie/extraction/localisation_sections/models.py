"""Structures de données et normalisation de texte du localisateur de sections.

Contient ``normalize_text`` et les dataclasses décrivant un élément visuel,
une entrée de table des matières, une section localisée et le mapping complet
d'un document.
"""
...
def normalize_text(text: str) -> str:
    ...
    Permet de faire correspondre « réglementation » avec « reglementation », etc.
...
class VisualTextElement:
    """Élément de texte avec ses caractéristiques visuelles."""
    ...
    def height(self) -> float:
        """Hauteur de l'élément en points."""
        return abs(self.y1 - self.y0)

    @property
    def width(self) -> float:
        """Largeur de l'élément en points."""
        return abs(self.x1 - self.x0)

    @property
    def is_likely_header(self) -> bool:
        """Indique si l'élément a les caractéristiques d'un titre."""
    ...
    def bbox_norm(self) -> list[float] | None:
        """Retourne la bbox normalisée [x0, y0, x1, y1] si la taille de page est connue."""
...
class LocatedSection:
    """Représente une section localisée dans le document."""
