...
_CHUNK_COMPARISON_VALIDATION_RETRY_MESSAGE = (
    ...
    "avoir text_t2 non vide, removed doit avoir text_t1 non vide. "
    "Si une seule face est remplie, utilise added (text_t1 vide) ou removed "
    "(text_t2 vide), jamais modified. Ne fusionne jamais plusieurs alignments "
    "dans un même changement."
)
...
class ChunkComparisonLLMChange(BaseModel):
    ...
    @classmethod
    def _coerce_one_sided_pair_diff_type(cls, data: Any) -> Any:
        """Remap modified/unchanged with a single side filled to added/removed.

        GPT sometimes labels a pure addition as modified while leaving text_t1
        empty (or a pure removal with empty text_t2). Normalize before the
        strict side-presence checks so a corrective LLM retry is not required.
        """
        if not isinstance(data, dict):
            return data
        diff_type = str(data.get("diff_type") or "").strip().lower()
        text_t1 = str(data.get("text_t1") or "").strip()
        text_t2 = str(data.get("text_t2") or "").strip()
        if diff_type not in {"modified", "unchanged"}:
            return data
        if text_t2 and not text_t1:
            return {**data, "diff_type": "added", "text_t1": ""}
        if text_t1 and not text_t2:
            return {**data, "diff_type": "removed", "text_t2": ""}
        return data
