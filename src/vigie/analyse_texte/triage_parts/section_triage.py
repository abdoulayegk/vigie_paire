...
def _triage_section_changes(
    *,
    ...
    user_prompt = (
        ...
        "d’application d’une exigence réglementaire déjà connue (simple report de date, "
        "sans changement de la nature de l’exigence), un déplacement "
        "identique, du formatage ou une reformulation sans nouveau fond sont non "
        "pertinents. Un report indéfini, un nouveau préavis d’application ou des "
        "révisions à une ligne directrice restent pertinents. Exception : le changement explicite du nom d’un comité ou "
