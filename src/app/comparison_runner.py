...
def _table_to_artifact(
    ...
    return TableArtifact(
        ...
        page_local_rank=getattr(table, "page_local_rank", None),
        page_table_count=getattr(table, "page_table_count", None),
        page_zone=getattr(table, "page_zone", None),
        y_top=getattr(table, "y_top", None),
        y_bottom=getattr(table, "y_bottom", None),
        y_center=getattr(table, "y_center", None),
        context_before=str(getattr(table, "context_before", "") or ""),
        context_after=str(getattr(table, "context_after", "") or ""),
        neighbor_above_distance=getattr(table, "neighbor_above_distance", None),
        neighbor_below_distance=getattr(table, "neighbor_below_distance", None),
    )
            ...
            if unmatched_added and unmatched_removed:
                ...
                for rem in unmatched_removed:
                    ...
                    for add in unmatched_added:
                        ...
                        cache_key = (t1_uid_r, t2_uid_a)
                        if cache_key in vision_pair_validation_cache:
                            vd = vision_pair_validation_cache[cache_key]
                        else:
                            ...
                            vision_pair_validation_cache[cache_key] = vd
                        tested_pairs += 1
